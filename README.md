...
아래 분담은 현재 구현된 코드 영역을 기준으로 정리했습니다.
...
첨부 ERD 초안에는 인터뷰, 진단, 학습 노트 등 확장 엔티티가 포함되어 있었지만, 현재 실제 구현 기준 ERD는 아래와 같습니다.
...
AI는 데이터 접근, 생성 등의 모든 기능 영역에서 백엔드가 읽어올 수 있는 "식별자"만을 사용하도록 하였고, 실제 DB로의 연결 및 생성 등 데이터를 직접적으로 다루는 작업은 모두 백엔드에서 맡도록 책임을 분리하였습니다.

#### 6. 코드 생성 이전 의사결정 포인트 리스트업 

"~한 코드 생성해줘" 수준에 머무르는 것이 아니라,
한 기능을 구현 및 고도화 하는데 있어 의사결정이 필요한 영역을 직접 판단 및 LLM을 통해 파악하고, 해당 부분에 대해 담당자/팀과 함께 논의하여 결정한 사안을 기반으로 코드를 생성하도록 했습니다. 
...
`--reset` 옵션을 통해 기존의 ChromaDB를 초기화 후 작업하도록 하여 기존 인코딩과의 중복 처리가 되는 것을 방지했습니다.
...
초기에는 `category` 중심 분류를 고려했지만, 실제 추천에는 더 작은 학습 개념 단위가 필요했습니다. 따라서 category는 관리/표시용으로 두고, 추천 기준은 `topic`으로 설계했습니다.

따라서 모든 강의자료 및 강의계획서 데이터는 `topics` 를 기준으로 연결되도록 할 수 있었습니다.
...
순수 LLM만 사용하면 실제 보유 데이터에 없는 강의나 자료를 만들어낼 위험이 있습니다. 반대로 키워드 검색만 사용하면 사용자 목적, 수준, 기간, 학습 스타일을 충분히 반영하기 어렵습니다. RAG로 실제 데이터 기반성을 확보하고, Agent로 입력 분석/검색/생성 단계를 분리했습니다. 

이러한 설계를 통해 디버깅 시에도 기능에 대한 전체 코드를 검토하는 것이 아니라 해당하는 세부 기능 단위의 코드 영역에서 빠르게 진행할 수 있도록 했습니다.
...
## 구현 과정 중 학습한 내용, 어려웠던 부분, 새로 배운 것들 및 느낀 점

여성욱 : 이번 프로젝트에서 가장 많은 고민과 배움이 있었던 부분은 **백엔드와 AI를 연결하는 구조를 설계하는 일**, 그리고 이를 뒷받침할 **ERD를 정의하는 과정**이었습니다.

ERD를 설계하면서 단순히 테이블을 나누는 데 그치지 않고, 어떤 데이터를 하나의 엔티티로 관리할지, 어떤 관계를 중간 테이블로 분리할지, 어디까지 정규화하는 것이 적절한지를 여러 선택지와 비교했습니다. 구조를 지나치게 단순화하면 중복과 확장성 문제가 생기고, 반대로 과도하게 분리하면 조회와 저장 과정이 복잡해질 수 있었기 때문에 서비스에 적절한 균형을 찾는 것이 중요했습니다. 이러한 의사결정 과정이 길게 느껴지기도 했지만, 각 구조의 장단점을 비교하며 더 나은 형태를 찾아가는 과정 자체는 꽤 즐거웠습니다.

또한 ERD는 처음 한 번 정의하고 끝나는 문서가 아니었습니다. 실제 사용자가 면담을 진행하고, 커리큘럼을 생성하고, 세부 학습 단계와 리소스를 확인하는 흐름을 구체적으로 생각할수록 기존 구조에서 부족한 관계나 분리해야 할 데이터가 보였습니다. 이에 따라 사용자의 사용 흐름과 향후 기능 확장 가능성을 고려하며 ERD를 계속 수정했습니다. 이를 통해 데이터베이스 설계는 기능 구현 이전의 독립된 작업이 아니라, 사용자 경험과 서비스 동작을 함께 반영해야 하는 과정이라는 점을 느꼈습니다.

백엔드와 AI를 연결할 때는 AI의 응답을 그대로 저장하지 않고, 백엔드가 이해할 수 있는 데이터 계약을 만드는 데 집중했습니다. AI가 토픽의 `slug`, 강의의 원본 행 번호, 학습 리소스의 외부 ID처럼 실제 DB 객체를 찾을 수 있는 식별자를 반환하게 하고, 백엔드에서는 응답 형식과 식별자의 유효성을 검증한 뒤 데이터를 저장하도록 역할을 분리했습니다.

이 과정에서 AI는 자연어 결과를 생성하는 역할을 담당하고, 백엔드는 그 결과가 서비스의 데이터 구조와 규칙에 맞는지를 책임져야 한다는 점을 배웠습니다. 결국 안정적인 AI 기능을 구현하기 위해서는 좋은 프롬프트뿐 아니라, 명확한 ERD와 식별자 설계, 응답 검증 및 저장 구조가 함께 필요했습니다.

이번 경험을 통해 ERD는 단순한 테이블 명세가 아니라 **사용자의 흐름, 백엔드의 처리 방식, AI의 응답 형식을 연결하는 서비스의 기반**이라는 점을 크게 느꼈습니다.


이창준 : 이번 프로젝트에서 저는 `topics` 데이터셋 구축 및 `AI 기능(RAG + Agent)` 구현을 맡았습니다. 

구현 과정 중 학습한 내용은, RAG 및 Agent 생태계에서 자주 활용되는 프레임워크에 대한 지식에 관련된 부분이었습니다. 

이 단계에서, 단순히 아는 것을 그대로 쓰는 게 아니라, 더욱 심화된 내용까지 이해하도록 노력하여 AI 서비스 개발 역량을 키우고자 했습니다.

 - 단순히 '로컬 프로젝트니까 `ChromaDB`를 사용하자'에서 그치는 것이 아니라, '프로젝트 내의 로컬 파일 기반으로 빠르게 벡터 DB를 구축할 수 있는 `ChromaDB`를 활용하자'와 같이, 구현한 방식에 대한 이유를 스스로 찾고 정립해보며 앞으로 나아갈 수 있었습니다. 

어려웠던 부분은, LLM을 활용하되 `매번 응답이 바뀌는` 것이 아닌 `일관성을 가진 안정된 서비스`를 구축하고자 했던 점입니다. 

클라우드 LLM 서비스(ChatGPT, gemini 등) 를 이용하다 보면, 같은 프롬프트를 제공하더라도, 모두 다 다른 형식의 응답을 제공하는 것의 불편함이 있었습니다. 이는 곧 저희 프로젝트의 시발점이 되기도 했던 만큼, 근본적인 이 문제에 대해 여러 방면의 해결방안을 탐색했던 것 같습니다. 

또한, 곧 이번 `WhereToGo` 프로젝트의 존재의의라고 판단했기에, AI 특강 2 기간에 접했던 `RAG + Agent` 실습을 기반으로 부족하지만 하나하나 공부해가며 구현을 진행한 것 같습니다. 

새로 배운 것들 및 느낀점 중 가장 핵심은, 'AI에게 무한한 자율성을 부여하는 것이 정답만은 아니다'라는 점이었습니다. 우리는 점점 AI에게 삶의 많은 영역을 맡기고, 그에 따른 책임과 권한 또한 전달하고 있습니다. 

그러나, 이로 인하여 원치않거나 초기의 목적, 의도와 상충되는 작업이 진행되기도 하는 모습을 보며, 왜 최신 LLM이 더욱 발전해갈 수록 `AI 가드레일, 윤리 검증 및 평가` 와 같은 영역이 강조되는 지를 느낄 수 있었던 것 같습니다. 

`Human-In-the-Loop` 가 추구하는 방식처럼, 이제는 개발자보다도 '관리자'의 입장에서 우리 서비스의 의도와 목적, 기능 및 구현 방안을 정확히 인지한 상태에서 핵심적인 의사결정을 진행하는 것이 앞으로 가져야 할 마인드셋임을 느꼈습니다. 

또한, AI는 '정말 사용하는 사람만큼 똑똑하다'는 주장에 동의하게 되는 순간이 많았다고 느꼈습니다. 단순하고 모호하게 전달할 수록 결과는 예상과 많이 달라졌습니다. AI와의 대화는 인간과의 대화와는 좀 더 다른 성격을 가진 영역이고, 따라서 이에 맞는 의사소통 방식을 더욱 익힐 필요가 있겠다고 느꼈습니다.